...
const BuildingsListingGridItem = ({
  ...
  >
    ...
    {/* Headline */}
    {title ? <h2 className={cls["shopping-card-title"]}>{title}</h2> : title}

    {/* Text */}
    {text ? <div className={cls["shopping-card-text"]}>{text}</div> : null}
    {status || price ? (
      <div className={cls["shopping-card-footer"]}>
        {status ? (
          <div className={cls["shopping-card-status"]}>{status}</div>
        ) : null}

        {price ? (
          <div className={cls["shopping-card-price"]}>{price}</div>
        ) : null}
      </div>
    ) : null}
  </Component>
